...
class DataProcessor:
    ...
    def scale(self,reverse=False):
        ...
        if not reverse:
            ...
            X_train_unscaled_num = pd.DataFrame(self.num_transformer.named_transformers_['num'].inverse_transform(self.X_train[self.num_cols]), columns=self.num_cols,index=self.X_train.index) 
            self.X_train = pd.concat([X_train_unscaled_num, self.X_train[self.cat_cols]], axis=1)
    ...
    def encode(self, reverse=False):
        if not reverse:
            self.X_train_encoded = self.cat_transformer.fit_transform(self.X_train[self.cat_cols])
            self.X_test_encoded = self.cat_transformer.transform(self.X_test[self.cat_cols])
            self.is_encoded = True
        else:
            self.X_test_encoded = pd.DataFrame(self.cat_transformer.named_transformers_['cat'].inverse_transform(self.X_test[self.cat_cols]), columns=self.cat_cols, index=self.X_test.index)
            self.is_encoded = False

        if self.is_encoded:
            feature_names = self.cat_transformer.named_transformers_['cat'].get_feature_names_out(self.cat_cols)
            encoded_cols_train = pd.DataFrame(self.X_train_encoded.toarray(), columns=feature_names, index=self.X_train.index)
            encoded_cols_test = pd.DataFrame(self.X_test_encoded.toarray(), columns=feature_names, index=self.X_test.index)
            self.X_train = pd.concat([self.X_train[self.num_cols], encoded_cols_train], axis=1)
            self.X_test = pd.concat([self.X_test[self.num_cols], encoded_cols_test], axis=1)
        else:
            self.X_train = pd.concat([self.X_train[self.num_cols], self.X_train_encoded], axis=1)
            self.X_test = pd.concat([self.X_test[self.num_cols], self.X_test_encoded], axis=1)
